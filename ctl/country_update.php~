<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST")
    {
            ...
            $rows = query("update countries set country=?, notes=?, user_id=?, changed=CURRENT_TIMESTAMP() where id=?",
				...
				{
					render("../view/country_update_form.php", ["title" => "Update Countries", "form_id" => "$rec_id", "message" => "Update was successful."]);        
        		}
    }
    else
    {
		...
		$_SESSION["rec_id"] = $id;
      render("../view/country_update_form.php", ["title" => "Update a Country",
            "form_id" => "$id", "message" => ""]);
    }
